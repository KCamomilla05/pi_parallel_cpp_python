# Скрипты и команды, использованные при выполнении вычислительных экспериментов
...
## 3. Скрипты проведения вычислительных экспериментов

Во всех сериях экспериментов использовались значения `n ∈ {10^6, 10^7, 10^8}`, `p ∈ {1,2,4}`.

Поскольку исследование выполнялось на ПК с ограниченными ресурсами, значения `p > 4` приводили бы к переподписке вычислительных ресурсов и не отражали бы реального аппаратного параллелизма системы.
...
* расчёт ускорения `S_p=T_1/T_p`;
* расчёт эффективности `E_p=S_p/p`;
